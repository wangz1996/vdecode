...
void test(const std::string& fname){
	...
	const int N=25;
	for(int i=0;i<N;i++){
		umap_hmh[i]=new TH1D(Form("hmh_%d",i),Form("hmh_%d",i),1000,0,100000);
		umap_hml[i]=new TH1D(Form("hml_%d",i),Form("hml_%d",i),1000,0,100000);
		umap_hbh[i]=new TH1D(Form("hbh_%d",i),Form("hbh_%d",i),1000,0,100000);
		umap_hbl[i]=new TH1D(Form("hbl_%d",i),Form("hbl_%d",i),1000,0,100000);
	}
